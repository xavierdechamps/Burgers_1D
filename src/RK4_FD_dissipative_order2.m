...
% non-linear terms
	diagm1 =   one_over_h * fourth * Un(ind(:,1),1);
	diagp1 = - one_over_h * fourth * Un(ind(:,3),1);
  
% subgrid terms
	diagm1  = diagm1 + (constant_sub * one_over_h) *   abs(Un(ind(:,2),1) - Un(ind(:,1),1));
	diag0   =        - (constant_sub * one_over_h) * ( abs(Un(ind(:,2),1) - Un(ind(:,1),1)) + abs(Un(ind(:,3),1) - Un(ind(:,2),1)) );
