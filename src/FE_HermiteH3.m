...
% Analytical evaluation of the derivatives of the shape functions at the given integration points
  for i=1:length(weight_gauss)
    d_shape_fct_vector(:,i) = get_deriv_shape_fct(eta_gauss(i),h) ;
  end
...
%                               Alpha_Pade ,mat_alpha,ind,d_shape_fct_vector,weight_gauss);
    ...
    [u(:,z),dynamic_smag_constant(i-1)] = ...
             RK5SSP_FE_HermiteH3 (u(:,z-1),deltat,h,N,M,MF,K,F,constant_sub,Filterin,indfilter,...
                               Alpha_Pade ,mat_alpha,ind,d_shape_fct_vector,weight_gauss);
...
%          hold off
        ...
        xlim([ 1 N ])
        minY = min( [ reference_spectrum(N,2) ; min( spectralEnergy(1:N)/nbrPointsStatistics ) ] );
...
% Stability criterion for explicit Runge Kutta 4
  ...
  standard_deviation = 100*std(dynamic_smag_constant)/mean_Smagorinsky
%  relative_error
...
function [y,smag_sub] = RK5SSP_FE_HermiteH3 (u,deltat,h,N,M,MF,K,F,constant_sub,filter,indfilter,...
...
function d_shape_fct = get_deriv_shape_fct(ksi,h)
...
%  -1 <=  ksi <= 1
%
	d_shape_fct = [        -1.5*(1+ksi) +  0.75*(1+ksi)^2   ;
	                h * ( 0.5 - (1+ksi) + 0.375*(1+ksi)^2 ) ;
	                        1.5*(1+ksi) -  0.75*(1+ksi)^2   ;
	                h * (  -0.5*(1+ksi) + 0.375*(1+ksi)^2)
	              ];
end
...
% unknown   u(i)  dui/dx  u(i+1)  du(i+1)/dx
% ind        3      4       5         6
  ...
  factor = constant_sub^2 * 4 ;
	for i = 1:length(weight) % Loop over the integration points
		deriv_u = deriv_shape(1,i) * u2 + deriv_shape(2,i) * du2 + deriv_shape(3,i) * u3 + deriv_shape(4,i) * du3 ;
