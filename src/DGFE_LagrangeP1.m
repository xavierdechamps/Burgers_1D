function DGFE_LagrangeP1(N,nu,constant_sub,filter,Alpha_Pade,L,time,nbrpointtemp,name,file_spectrum)
...
%              RK4_DGFE_Lagrangep1(u(:,z-1),deltat,N,M,nu,h,F,constant_sub,ind,filter,Alpha_Pade ,mat_alpha);
    [u(:,z),dynamic_smag_constant(i-1)] = ...
              RK3SSP_DGFE_Lagrangep1(u(:,z-1),deltat,N,M,nu,h,F,constant_sub,ind,filter,Alpha_Pade ,mat_alpha);
%    [u(:,z),dynamic_smag_constant(i-1)] = ...
%              RK5SSP_DGFE_Lagrangep1(u(:,z-1),deltat,N,M,nu,h,F,constant_sub,ind,filter,Alpha_Pade ,mat_alpha);
...
function [y,smag_sub] = RK3SSP_DGFE_Lagrangep1 (u,deltat,N,M,nu,h,F,constant_sub,ind,filter,alpha,mat_alpha)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Second step
  ...
  Un2 = 0.75*u + 0.25*Un1 + 0.25*deltat*k2 ;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Third step
  ...
  Un3 = u/3.0 + 2.0*Un2/3.0 + 2.0*deltat*k3/3.0 ;
  
  % Apply slope limiter
%  y = Un3 + F ;
  y = slope_lim(Un3,N,h) + F ;
  
end

function [y,smag_sub] = RK5SSP_DGFE_Lagrangep1 (u,deltat,N,M,nu,h,F,constant_sub,ind,filter,alpha,mat_alpha)
% Temporal integration of the 1D Burgers equation with an explicit 4 steps Runge-Kutta scheme
% Spatial discretization with linear Lagrange elements
% 
% The equation to be solved is 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Fifth step
  ...
  % Apply slope limiter
%  y = Un5 + F ;
  y = slope_lim(Un5,N,h) + F ;
  
end

function [y,smag_sub] = RK4_DGFE_Lagrangep1 (u,deltat,N,M,nu,h,F,constant_sub,ind,filter,alpha,mat_alpha)
% Temporal integration of the 1D Burgers equation with an explicit 4 steps Runge-Kutta scheme
% Spatial discretization with linear Lagrange elements
% 
% The equation to be solved is 
...
function u_lim = slope_lim(u,N,h)
...
%  u_tilt2_mod = minmod_vec(u_tilt2, delta_uplus, delta_uminus);
  ...
  M = 0 ; % best fit with the reference curve
