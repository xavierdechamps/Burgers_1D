...
% Skew-symmetric form = energy conservative form (see the appendix in Fauconnier - Dick)
  vecC = ( ( vpi + vpip1 ) .* ( 3*Un(ind(:,3)) - Un(ind(:,2)) ) + ...
           ( vmi + vmip1 ) .* ( 3*Un(ind(:,4)) - Un(ind(:,5)) ) - ...
           ( vpi + vpim1 ) .* ( 3*Un(ind(:,2)) - Un(ind(:,1)) ) - ...
           ( vmi + vmim1 ) .* ( 3*Un(ind(:,3)) - Un(ind(:,4)) ) ) / ( 6 * h ) ;
...
%           vmim1.*(3*Un(ind(:,3))-Un(ind(:,4))) ) * 0.25 / h ;
endfunction
